...
<div class="main-content dashboard-wrapper flex-grow-1">
    <div class="form-container" style="max-width: 1200px;">
        ...
        <div class="dash-header mb-4 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
            <div>
                <a href="index.php?produk_id=<?= $followup['produk_id'] ?>" class="text-muted text-decoration-none fw-bold" style="font-size: 0.85rem;">
                    <i class="fas fa-arrow-left me-1"></i> Kembali ke Sequence
                </a>
                <h1 class="dash-title mt-2 d-flex align-items-center gap-2">
                    Edit Pesan Follow-up
                </h1>
            ...
            <div class="d-flex gap-2">
                <a href="delete.php?id=<?= $followup['id'] ?>" class="btn btn-light border-danger text-danger fw-bold rounded-pill px-3" onclick="return confirm('Yakin ingin menghapus pesan ini dari sequence?')">
                    <i class="fas fa-trash-alt"></i>
                </a>
                <button type="submit" form="followupForm" class="btn btn-dark fw-bold rounded-pill px-4 btn-submit">
                    <i class="fas fa-save me-2"></i> Update Pesan
                </button>
            </div>
        </div>
        ...
        <form method="POST" id="followupForm" class="row g-4">
            ...
            <div class="col-lg-7">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control-editorial fw-bold text-dark" name="nama_pesan" 
                                   value="<?= clean(post('nama_pesan', $followup['nama_pesan'])) ?>" required 
                                   placeholder="Contoh: Hari ke-1 (Tanya Kabar)">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Sequence Ke- <span class="text-danger">*</span></label>
                            <input type="number" class="form-control-editorial text-center fw-bold" name="urutan" 
                                   value="<?= post('urutan', $followup['urutan']) ?>" min="1" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Status</label>
                            <select class="form-control-editorial fw-bold" name="status" style="appearance: auto;">
                                <option value="aktif" <?= post('status', $followup['status']) === 'aktif' ? 'selected' : '' ?>>🟢 Aktif</option>
                                <option value="nonaktif" <?= post('status', $followup['status']) === 'nonaktif' ? 'selected' : '' ?>>🔴 Nonaktif</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control-editorial fw-bold" name="delay_value" 
                                   value="<?= post('delay_value', $followup['delay_value']) ?>" min="1" required>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-control-editorial fw-bold" name="delay_unit" style="appearance: auto;">
                                <?php foreach (getDelayUnits() as $value => $label): ?>
                                    <option value="<?= $value ?>" <?= post('delay_unit', $followup['delay_unit']) === $value ? 'selected' : '' ?>>
                                        <?= $label ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                ...
                <div class="panel-editorial mb-4" style="border-left: 4px solid #10B981;">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control-editorial fw-bold" name="tipe_pesan" onchange="toggleImageField()" style="appearance: auto; max-width: 250px;">
                            <option value="pesan" <?= post('tipe_pesan', $followup['tipe_pesan']) === 'pesan' ? 'selected' : '' ?>>Teks Saja</option>
                            <option value="pesan_gambar" <?= post('tipe_pesan', $followup['tipe_pesan']) === 'pesan_gambar' ? 'selected' : '' ?>>Teks + Gambar</option>
                        </select>
                    </div>
                    ...
                    <div id="imageField" class="mb-4 p-3 bg-light rounded-3 border" style="display: none; transition: all 0.3s;">
                        ...
                        <input type="url" class="form-control-editorial bg-white" name="link_gambar" id="linkGambar"
                               value="<?= clean(post('link_gambar', $followup['link_gambar'] ?? '')) ?>" 
                    ...
                    <textarea class="form-control-editorial fw-medium" name="isi_pesan" id="isiPesan" rows="10" required 
                              ...
                              oninput="updatePreview()"><?= clean(post('isi_pesan', $followup['isi_pesan'])) ?></textarea>
                </div>
                
                <div class="text-muted text-center" style="font-size: 0.75rem;">
                    <i class="fas fa-clock me-1"></i> Dibuat: <?= formatDate($followup['created_at'], 'd M Y, H:i') ?> &bull; Terakhir diubah: <?= formatDate($followup['updated_at'], 'd M Y, H:i') ?>
                </div>
