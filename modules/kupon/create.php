...
if (isPost()) {
    ...
    $kuota = post('kuota');
    $tgl_mulai = str_replace('T', ' ', post('tgl_mulai')) . ':00';
    $tgl_selesai = str_replace('T', ' ', post('tgl_selesai')) . ':00';
    ...
    // Gunakan execute dari init.php
    $insert = execute($query, [$kode_kupon, $tipe_diskon, $nilai_diskon, $max_potongan, $produk_id, $kuota, $tgl_mulai, $tgl_selesai, $is_active]);

    if ($insert) {
        setMessage("Kupon berhasil ditambahkan!", "success");
        redirect('index.php');
    } else {
        setMessage("Gagal menyimpan kupon. Pastikan kode kupon belum pernah digunakan.", "danger");
    }
}
